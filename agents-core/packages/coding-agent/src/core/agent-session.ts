...
type BeforeToolCallHook = NonNullable<Parameters<Agent["setBeforeToolCall"]>[0]>;
...
export class AgentSession {
	...
	private readonly _beforeToolCallHooks = new Set<BeforeToolCallHook>();
	...
	private _installAgentToolHooks(): void {
		this.agent.setBeforeToolCall(async (context, signal) => {
			...
			if (!runner?.hasHandlers("tool_call")) {
				for (const hook of this._beforeToolCallHooks) {
	...
	addBeforeToolCall(handler: BeforeToolCallHook): () => void {
		this._beforeToolCallHooks.add(handler);
		return () => {
			this._beforeToolCallHooks.delete(handler);
		};
	}
