...
export class SelectList implements Component {
	...
	selectRenderedLine(renderedLineIndex: number): SelectItem | null {
		const { startIndex, endIndex } = this.getVisibleRange();
		const itemCount = endIndex - startIndex;
		if (renderedLineIndex < 0 || renderedLineIndex >= itemCount) {
			return null;
		}

		this.selectedIndex = startIndex + renderedLineIndex;
		this.notifySelectionChange();
		return this.filteredItems[this.selectedIndex] ?? null;
	}

	activateSelected(): SelectItem | null {
	...
	moveSelection(delta: number): SelectItem | null {
